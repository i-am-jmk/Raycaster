...
int main() {
	...
	Texture err_texture;
	err_texture.loadFromFile("images/error.png");
	Image error_image = err_texture.copyToImage();
	
	ifstream ifs("levels/test.json");
    ...
    for (int i = 0; i < ROWS; i++)
        for (int j = 0; j < COLUMNS; j++) {
            wall_tiles[i][j] = document["wall_tiles"][i][j].get<int>();
            floor_tiles[i][j] = document["floor_tiles"][i][j].get<int>();
            roof_tiles[i][j] = document["roof_tiles"][i][j].get<int>();
        }
    ...
    for (const auto& entity : document["entities"])
        entities.push_back({
            entity["image"].get<int>(),
            entity["script"].get<int>(),
            entity["x"].get<float>(),
            entity["y"].get<float>()
        });

    float player_x = document["player"]["x"].get<float>();
    float player_y = document["player"]["y"].get<float>();
    float player_r = document["player"]["r"].get<float>();
    float speed = document["player"]["speed"].get<float>();
